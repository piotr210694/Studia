...
session_start(); //start sesji
	...
	/* sprawdzanie loginu */
	$ins = @mysql_query("SELECT login FROM `uzytkownik` WHERE login='$elogin'") or die(mysql_error());
	while ($wiersz=mysql_fetch_array($ins)) 
	{
		$loginspr = $wiersz['login'];
	}
	
	if($_POST['elogin'] AND $_POST['eemail'] )
	{
		$ins = @mysql_query("UPDATE `uzytkownik` SET `login` = '$elogin',`email` = '$eemail', `telefon` = '$ephone', `imie` = '$ename', `nazwisko` = '$esurname' WHERE `id` = '$id';") or die(mysql_error());
		$_SESSION['login']=$elogin;
		echo '<div class="alert alert-success">';
		echo '<strong>Sukces!</strong> Nastąpiła edycja danych!</div>';
	}
	// else if($_POST['elogin']==$loginspr )
	// {
		// $komunikat = '<div class="alert alert-danger"><strong>Błąd!</strong> Podany login jest już w bazie! Popraw to!</div>';
		// echo $komunikat;
	// }
	else
	{
